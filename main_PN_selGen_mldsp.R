...
# Read data from file
f1<-read.csv(file="myCluster3rare_BabayanEtAl_VirusData.csv",header=T) 
...
fmldsp<-read.csv(file="group9_disMat_category_new.csv",header=T) 
# Feature definition
...
t<-15 #2 #there are 2 NAs. threshold for minimum sample size of groups 
...
groups_mldsp = c("A", "Ba", "B", "C", "F", "I", "Pl", "P", "R") 
# sample split of training/test to get counts in each
trains<-f_st3 %>% group_by(Reservoir) %>% # dataset for training that contains genomic features
...
rm(f,f1,f2,fis,orp)#rm(f,f1,f2,fis,rar,orp)
...
nfeatures<-length(gen.feats)+ntax+ntax 
...
for (i in 1:nloops){
    ...
    f1_train_pre<-merge(trains,blast.uc,by.x="Genbank.accession",by.y="id",all.x=F,all.y=T) # merge trains and blast.uc with support scores of host categories
    f1_train<-merge(f1_train_pre,fmldsp,by.x="Genbank.accession",by.y="AcNmb",all.x=F,all.y=F) 
    set<-c("Reservoir", gen.feats, bp, groups_mldsp) 
    ...
    f1_test_pre<-merge(tests,blast.uc,by.x="Genbank.accession",by.y="id",all.x=F,all.y=T) 
    f1_test<-merge(f1_test_pre,fmldsp,by.x="Genbank.accession",by.y="AcNmb",all.x=F,all.y=F) 
    ...
    f1_opt_pre<-merge(optims,blast.uc,by.x="Genbank.accession",by.y="id",all.x=F,all.y=T) 
    f1_opt<-merge(f1_opt_pre,fmldsp,by.x="Genbank.accession",by.y="AcNmb",all.x=F,all.y=F) 
    ...
    f1_orphan_pre<-merge(orphans,blast.uc,by.x="Genbank.accession",by.y="id",all.x=T,all.y=T,sort=F) 
    f1_orphan<-merge(f1_orphan_pre,fmldsp,by.x="Genbank.accession",by.y="AcNmb",all.x=F,all.y=F)
    set<-c(gen.feats, bp, groups_mldsp) 
    ...
    # Clean up
    rm(f1_train,f1_test,f1_opt,f1_orphan) #rm(f1_train,f1_test,f1_opt,f1_orphan,f1_rare) 
    ...
    max_models = 500, # 100, # 500, 
    stopping_rounds=10,
    stopping_metric="misclassification",
    stopping_tolerance=1e-3)

    gbm_grid <- h2o.grid("gbm", x = x, y = y, # train models and find the best parameter combination
    grid_id = "gbm_grid",
    training_frame = train,
    validation_frame = opt,
    seed = 1,
    hyper_params = gbm_params,
    search_criteria = search_criteria) # end of function h2o.grid()
    ...
    # Save mode 
    #model_path <- "C:\\Users\\Bill\\Documents\\Rproject\\ViralHostPredictor\\model3parts"
    model_path <- getwd()
    h2o.saveModel(object=best_gbm, path=model_path, force=TRUE) # save model to hard drive
    name <- file.path(model_path, i) # loop id as name of the best model
    file.rename(file.path(model_path, best_gbm@model_id), name) # change model name
...
write.csv(vimps,file="Reservoir_PN+SelGen50+MLDSP_FI.csv",row.names = T) 
write.csv(accs,file="Reservoir_PN+SelGen50+MLDSP_out.csv",row.names=F) 
