...
%read fasta files
dataSet = 'MyViruses'; %dataSet = 'Primates';
selectedFolder = dataSet;
fprintf('Reading sequences .... \n');
[AcNmb, Seq, numberOfClusters, clusterNames, pointsPerCluster, categories] = readFasta(dataSet); %[AcNmb, Seq, numberOfClusters, clusterNames, pointsPerCluster] = readFasta(dataSet);
totalSeq = length(Seq);
...
[ weightOfCategories, STop, WTop, ITop, CTop ] = doWeightOfCategories( disMat, totalSeq, categories, numberOfClusters, clusterNames, 'mydisMatCategoryFile.csv');
%%%%%%%%%%%%%%%%%%%%
...
for i=1:numberOfClusters
    for j=1:pointsPerCluster{i}
        a=[a; i]; % generate label info Y
    end
end
ATestlg = [disMat a]; % training set with X and label Y
rng(15,'twister');

alabels = a; % specify label Y for model training
...
%%%%%%%%%%%%%%%%%%%% self added here can test model performance based on PN
PNgroup = readtable('DataBase/group9_PN_input.csv');
PNgroup = table2array(PNgroup);
[accuracyPN, avg_accuracyPN, clNamesPN] = classificationCodePN(PNgroup,alabels, folds, totalSeq);
accPN = [accuracyPN avg_accuracyPN];
s.ClassifierModel=cellstr(clNamesPN.');
s.Accuracy=cell2mat(accPN).';
ClassificationAccuracyScores = struct2table(s)
...
testDisMat = newSeqClassify('EPI_ISL_402131_N', mLen, disMat, lg, alabels, clusterNames);
testDisMat = reshape(testDisMat, [totalSeq 1]);
[ testWeightOfCategories, testSTop, testWTop, testITop, testCTop ] = doWeightOfCategories( testDisMat, 1, categories, numberOfClusters, clusterNames, 'mydisMatTest.csv', 1);
